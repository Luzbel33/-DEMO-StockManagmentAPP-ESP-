...
addInventory = () =>{
    ...
    let totalinventory = JSON.parse(localStorage.getItem(pageUrl + "_script16"));
    ...
    if (product == "" || product == null) {
        ...
        localStorage.setItem(pageUrl + "_script16", JSON.stringify(totalinventory))
...
editTitleButton.addEventListener('click', function() {
    ...
    if (newText != null) {
        ...
        let pageUrl = window.location.href;
        let invTitleKey = pageUrl + "_scriptTitle_16" + invTitle; // unique key for each invTitle value
...
let pageUrl = window.location.href;
let invTitleKey = pageUrl + "_scriptTitle_16" + invTitle;
...
uploadInventory = () => {
    ...
    reader.onload = function(event) {
      ...
      localStorage.setItem(pageUrl + "_script16", JSON.stringify(inventory));
      window.location.reload();
    };
...
clearButton = () => {
    if (confirm("Do you want to clear all your inventory data ? This action cannot be undone")) {
        localStorage.removeItem(window.location.href + "_script16");
        window.location.reload();
    }
}
